...
const lookUpStar = async () => {
  const instance = await StarNotary.deployed();
  let idToken = document.getElementById("starToken").value;
  let startName = await instance.lookUptokenIdToStarInfo(idToken, { from: account });
  App.setStarName("Star name: " + startName + ".");
}
...
const App = {
  ...
  setStarName: function (message) {
    const status2 = document.getElementById('statusName')
